...
import i18n
from app_paths import migrate_legacy_cache, migrate_legacy_data
from utils.exporter.template import TypeDefinitionTemplate
from utils.exporter.type_registry import TypeRegistry
from version import APP_NAME, GITHUB_URL, resource_path
...
class SheetToConfigUpgradeTests(unittest.TestCase):
    ...
    def test_catalogs_have_identical_keys_and_placeholders(self):
        ...
        for locale_id in i18n.SUPPORTED_LOCALES:
            path = ROOT / "i18n" / "catalogs" / f"{locale_id}.json"
            pairs = json.loads(
