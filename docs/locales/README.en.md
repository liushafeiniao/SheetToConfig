...
<p align="center">
  ...
  <a href="https://github.com/liushafeiniao/SheetToConfig/releases"><img alt="Current version 1.0.0" src="https://img.shields.io/badge/version-1.0.0-00D4AA?style=flat-square"></a>
...
- Empty values are handled according to the target field's real type; a missing sheet, field, or ID fails validation.
...
<details>
<summary><strong>Protobuf export</strong> — <code>.pb</code> generates a same-named <code>.proto</code>; superset schema and the breaking-change switch</summary>
...
The desktop UI forbids breaking protocol changes by default: an incompatible schema change raises an error immediately. An incompatible rebuild is allowed only after explicitly enabling "Allow breaking Protobuf schema changes", which takes effect as soon as it is checked; published protocols should still have their `.proto` diff reviewed.
...
| Asset root | No | Validates that `path()` results stay in bounds and the files really exist; when empty, validation falls back to the client output directory and emits a warning |
...
- Current version: `1.0.0` in [`sheet_to_config/version.py`](../../sheet_to_config/version.py)
