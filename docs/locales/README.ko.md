...
<p align="center">
  ...
  <a href="https://github.com/liushafeiniao/SheetToConfig/releases"><img alt="현재 버전 1.0.0" src="https://img.shields.io/badge/version-1.0.0-00D4AA?style=flat-square"></a>
...
내장 타입은 `int`, `float`, `string`, `bool`, `bytes`, `text_key`, 1~3차원 목록, 딕셔너리, 경로 `path()`, 워크북 간 ID 참조를 포함합니다. 첫 내보내기에서 자동 생성되는 `TypeDefinition.xlsx`에 전체 목록과 예제가 있습니다. 복합 타입도 TypeDefinition에서 조합 표현식으로 확장할 수 있습니다.

Enum은 3열 TypeDefinition에서 정의하며 새로운 Schema를 추가하지 않습니다. `enum(string,white,green,blue)`와 `enum(int,1,2,3)`는 먼저 기본 타입으로 엄격하게 변환한 뒤 허용 값을 검증합니다. 내보낸 값은 원래 문자열이나 정수를 유지하며 이름-숫자 매핑을 하지 않습니다.
...
- 빈 값은 대상 필드의 실제 타입을 따릅니다. 표·필드·ID가 없으면 검증에 실패합니다.
...
<details>
<summary><strong>Protobuf 내보내기</strong> — <code>.pb</code>는 같은 이름의 <code>.proto</code>를 생성, 상위 집합 프로토콜과 파괴적 변경 스위치</summary>
...
데스크톱 UI는 기본적으로 파괴적인 프로토콜 변경을 금지합니다. 호환되지 않는 스키마 변경이 감지되면 바로 오류를 보고합니다. 「호환성을 깨는 Protobuf 스키마 변경 허용」을 명시적으로 선택해야만 호환되지 않는 재구성이 허용되며, 선택하는 즉시 적용됩니다. 이미 배포된 프로토콜은 여전히 `.proto` diff를 확인해야 합니다.
...
| 리소스 루트 | 아니요 | `path()` 결과가 범위를 벗어나지 않았고 파일이 실제로 존재하는지 검증; 비워 두면 클라이언트 출력 디렉터리로 대체하여 계속 검증하고 경고 표시 |
...
- 현재 버전: [`sheet_to_config/version.py`](../../sheet_to_config/version.py)의 `1.0.0`
